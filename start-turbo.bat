...
cd /d "%V2_ROOT%"
title Computer Dynamics v2 (Turbopack)

echo.
echo  Computer Dynamics v2 - Portal + Express API + Security + License + Cloudflare (Turbopack)
echo  ==========================================================================================
echo.
...
echo.
echo  Starting portal ^(Turbopack^) + Express API + security worker + license API ^(npm run dev:all:turbo^)...
echo  Note: Turbopack portal waits for Express API on port %API_PORT% before binding to %APP_PORT%.
set PYTHONUTF8=1
set PYTHONIOENCODING=utf-8
start "Computer Dynamics v2 - Portal + API + Security + License (Turbopack)" /D "%V2_ROOT%" cmd /k "npm run dev:all:turbo"

echo  Waiting for Express API http://127.0.0.1:%API_PORT%/api/health/live ...
call "%V2_ROOT%\scripts\wait-api-ready.bat" %API_PORT% 360 15
if errorlevel 1 (
  pause
  exit /b 1
)
...
echo  Waiting for portal http://127.0.0.1:%APP_PORT%/api/health/live ^(Next proxy^)...
call "%V2_ROOT%\scripts\wait-portal-ready.bat" %APP_PORT% 360 20
if errorlevel 1 (
  pause
  exit /b 1
)
...
if %WAIT_SEC% geq 90 (
  ...
  echo            Check the dev:all:turbo window for [security] errors.
  goto wait_license
)
goto wait_worker
:worker_ready
...
echo.
echo  ========================================
echo   v2 is running (Turbopack dev)
echo  ========================================
...
echo    Security:      worker via dev:all:turbo ^(Settings - Security for status^)
if "%TUNNEL_OK%"=="1" (
...
echo.
echo  KEEP OPEN:
echo    - Computer Dynamics v2 - Portal + API + Security + License (Turbopack)
