...
import { sendQuoteEmail } from '@web/lib/accounting';
...
export async function POSTHandler(ctx: ApiContext): Promise<ApiResult> {
  try {
    ...
    const quote = await sendQuoteEmail(id, body.clientEmail, origin, session.id);
    if (!quote) {
      return { status: 404, body: { success: false, message: 'Quote not found' } };
    }
    return { status: 200, body: { success: true, message: 'Quote email sent', quote, emailSent: true } };
  } catch (error) {
