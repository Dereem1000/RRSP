...
cd /d "%V2_ROOT%"
title Computer Dynamics v2 - Production

echo.
echo  Computer Dynamics v2 - PRODUCTION
echo  ==================================
echo.
...
echo  Running production preflight (secrets audit)...
call npm run preflight:production
if errorlevel 1 (
  echo.
  echo  Preflight failed — fix blockers above before going live.
  pause
  exit /b 1
)

echo.
echo  Building production bundles ^(existing servers stay online until build succeeds^)...
call npm run build
if errorlevel 1 (
  echo.
  echo  ERROR: Build failed.
  echo  Existing servers were NOT stopped — the current deployment is still running.
  pause
  exit /b 1
)

if not exist "%V2_ROOT%\apps\web\.next" (
  echo.
  echo  ERROR: Next.js build output missing at apps\web\.next
  echo  Existing servers were NOT stopped.
  pause
  exit /b 1
)

if not exist "%V2_ROOT%\packages\security\dist\worker-cli.js" (
  echo.
  echo  ERROR: Security worker build output missing.
  echo  Existing servers were NOT stopped.
  pause
  exit /b 1
)
...
echo  Stopping previous servers ^(new build passed^)...
call "%V2_ROOT%\scripts\stop-v2-services.bat"

echo  Starting portal + Express API + security worker + license API (production)...
start "Computer Dynamics v2 - Production" /D "%V2_ROOT%" cmd /k "set NODE_ENV=production&& npm run start:production:runtime"
...
set /a WAIT_SEC+=3
if %WAIT_SEC% geq 90 (
  echo  ERROR: Security worker did not report online within 90 seconds.
  echo         Check the Production window for [security] errors.
  pause
  exit /b 1
)
goto wait_worker
...
REM --- Mini (when docked; preflight already verified token + MINI_PUBLIC_MODE) ---
set "MINI_DOCKED=0"
...
if "%MINI_DOCKED%"=="1" (
  echo  ERROR: Mini is docked but start_mini_headless.bat was not found at %MINI_INSTALL_PATH%
  pause
  exit /b 1
)
goto after_mini
...
set /a WAIT_SEC+=2
if !WAIT_SEC! geq 120 (
  echo  ERROR: Mini is docked but did not start within 120 seconds.
  echo          Check Mini runtime/local.env and the Mini AI Core window.
  pause
  exit /b 1
)
goto wait_mini_loop
...
if "%TUNNEL_OK%"=="1" (
  ...
  echo  WARNING: Cloudflare tunnel skipped.
)

echo.
echo  ========================================
echo   v2 PRODUCTION is running
echo  ========================================
...
echo    Security:      worker via start:production ^(Settings - Security for status^)
if "%TUNNEL_OK%"=="1" (
...
if "%MINI_DOCKED%"=="1" echo    Mini local:    http://127.0.0.1:%MINI_PORT%
echo.
echo  KEEP OPEN:
echo    - Computer Dynamics v2 - Production
if "%TUNNEL_OK%"=="1" echo    - Cloudflare Tunnel - Computer Dynamics v2
if "%MINI_DOCKED%"=="1" echo    - Mini AI Core
echo.
