  ...
  const CDDemos = {
    ...
    async load() {
      ...
      try {
        ...
        if (mRes.ok) this.manifest = await mRes.json();
    ...
    open(slug) {
      ...
      if (!url) {
        ...
        if (!demos.length) {
          alert(
            "No demos in manifest. In MultiServer: Start the demo, then Sync website, and restart the CD server."
          );
          ...
          alert(
            "Live demo is not available right now. Start it in MultiServer, then refresh this page."
          );
        }
        return;
      }
